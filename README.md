...
**Status: pre-alpha.** HTTP/1.1 requests work over TCP and over TLS today, so `http://` and `https://` both work. Streaming works in both directions: `client.stream()` returns as soon as the head has arrived and the body is read off the wire a chunk at a time with `iter_bytes`, `iter_text` or `iter_lines`, and `content_stream=` sends a request body that is pulled as it is written. HTTP/2 works over TLS when you ask for it with `Client(http2=True)`, which offers `h2` in the handshake and speaks it if the server agrees, though a connection still carries one request at a time rather than multiplexing. `AsyncClient` works over `http://` and `https://`, with `gather` for sending several requests at once and streaming through `aiter_bytes` and the rest, though it speaks HTTP/1.1 only and does not offer `h2` in the handshake. A compressed response is decoded on the way through: `Accept-Encoding` says `gzip, deflate, br, zstd`, each is undone through the library already on the machine, zlib for the first two and libbrotlidec and libzstd for the other two, a machine missing one of those asks for less rather than failing, and every decoded body has a bound on how large it is allowed to become. Forward proxying works with `Client(proxy=Proxy("http://localhost:3128"))`, which sends `http://` requests in absolute form and opens a `CONNECT` tunnel for `https://` ones, with the TLS handshake running inside the tunnel to the real server and `Proxy-Authorization` built from any credentials in the proxy URL. A SOCKS5 proxy works the same way, `Client(proxy=Proxy("socks5://localhost:1080"))`, with both the no auth and the username and password methods and the target's name resolved at the proxy rather than locally. The async client forwards `http://` through an HTTP proxy too, though it cannot tunnel. Traffic can be split between transports with `mounts=`, which routes by URL pattern, so some hosts go through a proxy and the rest go direct, one domain is answered by a mock while everything else goes out for real, and a scheme can be refused outright with `blocked()`. `HTTP_PROXY`, `HTTPS_PROXY`, `ALL_PROXY` and `NO_PROXY` are read from the environment unless `trust_env=False`, with curl's matching rules for `NO_PROXY` including address ranges, and with the CGI check that stops a request header from choosing where a server sends its own traffic. See the [roadmap](docs/roadmap.md) for what is landing and when, and [limitations](docs/limitations.md) for what is not here yet in one list. Do not use this in production.
...
`https://` works, over HTTP/1.1, running the same handshake and the same certificate checks as `Client` with the waiting lifted one level up. What it will not do yet is HTTP/2, and it says so by never offering `h2` rather than by offering it and then not speaking it. `close()` and `aclose()` are the same call, since nothing about closing a client suspends. See [async](docs/async.md) for the whole picture, including what Mojo 1.0.0 does and does not allow a coroutine to do.
...
| Proxies | HTTP, CONNECT tunnelling, SOCKS5, per pattern mounts, environment configuration |
...
What is not there yet is collected in [limitations](docs/limitations.md), and the short version is that the async client speaks HTTP/1.1 only and HTTP/2 carries one request at a time per connection.
