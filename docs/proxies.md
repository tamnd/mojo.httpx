...
All of that is written here, though, on both sides of the connection, so `pixi run interop-proxy` runs the same requests through Squid, tinyproxy, mitmproxy and Dante in Docker instead. Forty seven cases, including the ones a proxy written for this repository could never be trusted to check: a real refused CONNECT from Squid, tinyproxy's own 407 arriving as a response when it is forwarded and as an error when it is not, Dante checking an RFC 1929 password against a real account, and mitmproxy actually intercepting a tunnel so that the certificate the client refuses is one somebody else's proxy chose. It needs Docker and it runs on the local fleet rather than in CI. [testing.md](testing.md) covers both.
