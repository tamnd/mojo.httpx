...
- `gather` raises the first failure and drops the other responses, the way `asyncio.gather` does by default. There is no `return_exceptions` equivalent, because the pool underneath reports a batch as one outcome rather than as a list of them. The same thing costs you per request timing: every response in a batch reports roughly the duration of the whole batch, since nothing records the moment an individual one finished.
