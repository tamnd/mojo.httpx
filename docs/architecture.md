...
## A pool is either a direct pool or a proxy pool

Decided once, when the pool is built, and not per request. That is httpcore's arrangement and the reason is connection reuse.

A pool files its idle connections by origin, and every forwarded request through a proxy goes to the same address whatever server it is aimed at. A pool that could proxy some requests and not others has two ways to get that wrong and no third option. File the connection under the target's origin and a direct request to the same host and port will be handed a connection that goes to the proxy. File it under the proxy's origin and the pool has to remember, per connection, which requests it is allowed to carry, which is the same decision made again in a worse place. Fixing it at the pool makes both impossible: every connection in a proxy pool goes to the proxy and every request from it goes through the proxy.

What the request needs on top of that is small and lives in `httpx._pool.proxy.route_through`, which both pools call. It returns a `Hop`: the origin to connect to, the request target form to write, and the proxy to tunnel through if this one has to be tunnelled. Those come back together because working one out without the other is how a request ends up asking a proxy for `/path`. `route_through` also puts the proxy's headers on a forwarded request, which is where `Proxy-Authorization` is added: below the event hooks and below the auth flow, because the credential belongs to the hop rather than to the request the caller wrote, and a hook that logs every request should not be printing it.

A `CONNECT` tunnel is the one case where the key is the target rather than the proxy, and that is the same rule rather than an exception to it. A connection is keyed by what is on the far end, and the far end of a tunnel is the server: it is a private pipe to one host and handing it to a request for another would send that request somewhere it never asked to go. So a proxy pool ends up holding connections filed under the proxy, for forwarded `http://` requests, alongside connections filed under each `https://` server it has tunnelled to. Nothing has to tell them apart, because within one pool the same origin always means the same way of getting there.
...
So M6 builds a loop where waiting is polling rather than being woken. Each waiter looks at its own descriptor with a zero timeout while the wait is young and hands the worker straight back, and falls through to a one millisecond `poll` once it has been waiting long enough that spinning would cost something. There is no shared reactor: a waiter has to be handed a worker before it can act on the news anyway, and that is exactly what the per waiter poll is already waiting for. The full decision, the numbers behind it, the rotation cost this design carries and the four toolchain limits that shaped the code are in [async.md](async.md). Everything under `httpx/_io/` stays internal precisely so the polling can become a wake-up later without a breaking release.
