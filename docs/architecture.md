...
A pool files its idle connections by origin, and every request through a forward proxy goes to the same address whatever server it is aimed at. A pool that could proxy some requests and not others has two ways to get that wrong and no third option. File the connection under the target's origin and a direct request to the same host and port will be handed a connection that goes to the proxy. File it under the proxy's origin and the pool has to remember, per connection, which requests it is allowed to carry, which is the same decision made again in a worse place. Fixing it at the pool makes both impossible: every connection in a proxy pool goes to the proxy and every request from it goes through the proxy.

What the request needs on top of that is small and lives in `httpx._pool.proxy.route_through`, which both pools call. It answers two things together, the origin to connect to and the request target form to write, because working one out without the other is how a request ends up asking a proxy for `/path`. It also puts the proxy's headers on the request, which is where `Proxy-Authorization` is added: below the event hooks and below the auth flow, because the credential belongs to the hop rather than to the request the caller wrote, and a hook that logs every request should not be printing it.
